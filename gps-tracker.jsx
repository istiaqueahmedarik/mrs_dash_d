...
function AnimatedMarker({
  ...
  useEffect(() => {
    if (isPaused) return  // pause animation when simulation is paused
  ...
  }, [currentIndex, isPaused, waypoints, map, onPositionChange])
...
export default function GPSTracker() {
  ...
  const [openAddModal, setOpenAddModal] = useState(false) // new state for add waypoint modal
  const [isPaused, setIsPaused] = useState(false) // new paused state

  useEffect(() => {
    fetch("/waypoints1.json")
      .then((res) => res.json())
      .then((data) => {
  ...
  const handlePositionChange = useCallback(
    (lat, lng) => {
      ...
      if (matchedWaypoint) {
        ...
        setIsPaused(true) // pause when matched
      }
    },
    [userWaypoints],
  )
  ...
  const mapCenter = currentPosition
    ...
    : (waypoints.length > 0 ? [waypoints[0].lat, waypoints[0].lng] : [23.837598, 90.358761
])
  ...
  // Helper function to calculate bearing (heading) in degrees
  ...
  return (
    <div className="w-full max-w-7xl flex">
      <div className="w-1/3 p-4">
        {/* Remove inline lat/lng input and add a button to trigger the add waypoint modal */}
      ...
      <div className="w-2/3">
        <MapContainer
          center={mapCenter}
          zoom={30} // adjusted to a normal zoom level
          scrollWheelZoom={true} // enable scroll wheel zoom
          style={{ height: "calc(100vh - 2rem)", width: "100%", zIndex: -10 }} // updated style to fill the container
          className="z-[-10]"
        >
          <TileLayer
            ...
            maxZoom={19}
          />
          {/* Render markers for user-added waypoints with marker.jpg */}
          {userWaypoints.map((wp) => (
            <Marker
              key={"user-" + wp.id}
              position={[wp.lat, wp.lng]}
              icon={L.icon({ iconUrl: "/marker.png", iconSize: [20, 20] })}
            />
          ))}
          {/* Render initial rover marker if not animating */}
          ...
          {/* Animated marker when animation is active */}
          {isAnimating && <AnimatedMarker waypoints={waypoints} onPositionChange={handlePositionChange} isPaused={isPaused} />}
          <Polyline positions={animatedPath} />
        </MapContainer>
      </div>
      <Dialog open={!!reachedWaypoint} onOpenChange={() => {}} className="z-9999">
        <DialogContent className="">
          <DialogHeader>
            <DialogTitle>Waypoint Reached</DialogTitle>
          </DialogHeader>
          <p>
            Waypoint {reachedWaypoint?.id !== undefined ? reachedWaypoint.id + 1 : ""} has been reached!
          </p>
          <p>
            Latitude: {reachedWaypoint?.lat.toFixed(6)}, Longitude: {reachedWaypoint?.lng.toFixed(6)}
          </p>
          <Button onClick={resumeAnimation} className="mt-2">Yes</Button>
        </DialogContent>
      </Dialog>
      {/* New Dialog for adding a waypoint */}
      <Dialog open={openAddModal} onOpenChange={setOpenAddModal} className="z-9999">
        {/* Updated modal style to constrain width */}
        <DialogContent className="">
          <DialogHeader>
